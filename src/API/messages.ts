...
//   };
  ...
  export const getAllMessages = async (): Promise<Message[]> => {
    try {
      ...
      const messages: Message[] = chatSnapshot.docs.map((doc) => doc.data() as Message);
      return messages;
    } catch (error) {
  ...
  export const getMessagesByUserId = async (userId: string): Promise<Message[]> => {
    try {
      const chatCollectionRef = collection(db, "commonChat");
      const userMessagesQuery = query(chatCollectionRef, where("userId", "==", userId));
      const querySnapshot = await getDocs(userMessagesQuery);
  
      const userMessages: Message[] = querySnapshot.docs.map((doc) => doc.data() as Message);
      return userMessages;
    } catch (error) {
  ...
  export const updateMessageInDB = async (messageId: string, updates: Partial<Message>) => {
    try {
      const messageDocRef = doc(db, "commonChat", messageId);
  ...
  export const deleteMessageInDB = async (messageId: string) => {
    try {
      const messageDocRef = doc(db, "commonChat", messageId);
